...
<head>
    ...
    <!--
    	相对路径：转发可能会造成相对路径失效(打开的资源的路径参考位置 和 访问的url地址的相对位置不一样)
    		开发时：参考当前文件所在的位置
    		浏览器解析时：参考浏览器打开资源的url地址解析相对路径

    	绝对路径：  浏览器解析时 自动会拼接服务器的地址前缀
    	base标签+相对路径： 以绝对路径的方式使用相对路径
    			有了base标签，页面中所有的相对路径不再参考相对位置，和base标签拼接

    	- 多个页面中都需要获取上下文路径，简化获取路径的方式，可以在服务器启动成功后，立即向application域中存入上下文路径
    		在访问页面时就可以直接获取路径
    	- 创建ServletContext的生命周期监听器，当服务器启动成功全局上下文对象创建后，立即向域中存入初始化参数
    		AppStartupListener
     -->
